...
class EmailID:
    ...
    def get_by_user(user_id):
        ...
        if db is None or not user_id:
            return []
        try:
            user_filter = {'$in': [ObjectId(user_id), str(user_id)]} if ObjectId.is_valid(str(user_id)) else str(user_id)
            return list(db[Collections.EMAIL_IDS].find({
                'user_id': user_filter,
                'is_active': True
            }).sort('created_at', 1))
        except Exception as e:
            print(f"⚠️ Error in get_by_user: {e}")
            return []
    ...
    def get_by_id(email_id):
        ...
        if db is None or not email_id:
            return None
        try:
            if ObjectId.is_valid(str(email_id)):
                return db[Collections.EMAIL_IDS].find_one({'_id': ObjectId(email_id)})
            return db[Collections.EMAIL_IDS].find_one({'email': str(email_id)})
        except Exception:
            return None
    ...
    def get_by_user_with_passwords(user_id):
        """Get all email IDs for a user with passwords"""
        db = MongoDB.get_db()
        if db is None or not user_id:
            return []
        try:
            ...
            print(f"⚠️ Error in get_by_user_with_passwords: {e}")
            return []
    ...
    def increment_sent_count(email_id):
        ...
        if db is None or not email_id:
            return False
        try:
            query = {'_id': ObjectId(email_id)} if ObjectId.is_valid(str(email_id)) else {'email': str(email_id)}
            ...
            print(f"⚠️ Error incrementing sent count for {email_id}: {e}")
            return False
    ...
    def reset_counts(user_id):
        ...
        try:
            query = {'user_id': ObjectId(user_id)} if ObjectId.is_valid(str(user_id)) else {'user_id': str(user_id)}
            ...
            print(f"⚠️ Error resetting counts for user {user_id}: {e}")
            return False
    ...
    def get_by_id(template_id):
        ...
        try:
            ...
            if template:
                ...
                template['signature_format'] = template.get('signature_format') or Template.DEFAULT_SIGNATURE_FORMAT
                if 'attachments' not in template or not isinstance(template['attachments'], list):
                    template['attachments'] = []
            return template
        except Exception as e:
            print(f"⚠️ Error getting template {template_id}: {e}")
            return None
...
class EmailLog:
    ...
    def create(user_id, sender_email_id, recipient_email, subject, status, error_message=None):
        ...
        uid = ObjectId(user_id) if user_id and ObjectId.is_valid(str(user_id)) else str(user_id or '')
        sid = ObjectId(sender_email_id) if sender_email_id and ObjectId.is_valid(str(sender_email_id)) else str(sender_email_id or '')
        ...
        try:
            ...
            print(f"⚠️ Error inserting EmailLog: {e}")
            return log_data
    ...
    def get_by_user_paginated(user_id, page=1, limit=100):
        ...
        if user_id:
            if ObjectId.is_valid(str(user_id)):
                query['user_id'] = {'$in': [ObjectId(user_id), str(user_id)]}
            else:
                query['user_id'] = str(user_id)
        try:
            return list(db[Collections.EMAIL_LOGS].find(query).sort('sent_at', -1).skip(skip).limit(limit))
        except Exception as e:
            print(f"⚠️ Error fetching paginated logs for user {user_id}: {e}")
            return []
    ...
    def get_all_paginated(page=1, limit=100):
        ...
        try:
            ...
            print(f"⚠️ Error fetching all logs paginated: {e}")
            return []
    ...
    def get_count(user_id=None):
        ...
        try:
            ...
            print(f"⚠️ Error counting logs: {e}")
            return 0
