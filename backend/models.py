...
class EmailID:
    ...
    def create(user_id, email_data):
        """Create a new email ID for a user with OAuth tokens"""
        db = MongoDB.get_db()
        if db is None:
            return None
        ...
        email_id_data = {
            ...
            'provider': email_data.get('provider', 'google'),
            'refresh_token': email_data.get('refresh_token', ''),
            'access_token': email_data.get('access_token', ''),
            'token_uri': email_data.get('token_uri', 'https://oauth2.googleapis.com/token'),
            'scopes': email_data.get('scopes', []),
            'token_expiry': email_data.get('token_expiry', 0),
            'emails_sent': 0,
            'is_active': True,
            'created_at': datetime.utcnow()
        }
