...
# In a production environment, OAUTHLIB_INSECURE_TRANSPORT must NOT be set.
# It is only enabled here if explicitly not in production (e.g. local testing on HTTP)
...
def delete_email_id(email_id):
    ...
    return jsonify({'error': 'Failed to delete email ID'}), 400

# ==========================
# OAUTH 2.0 GOOGLE
# ==========================

@user_bp.route('/api/auth/google/login')
@require_login
def google_login():
    try:
        ...
        print("\n--- After authorization_url() ---")
        print("state:", state)
        print("flow.code_verifier:", getattr(flow, 'code_verifier', None))
        print("flow.redirect_uri:", getattr(flow, 'redirect_uri', None))
        
        session['oauth_state'] = state
        
        # PKCE: Save code verifier if generated by the library
...
@require_login
def google_callback():
    print("***** GOOGLE CALLBACK ENTERED *****")
    print(__file__)
    print(request.args)
    
    import os
    print("\n--- In google_callback() BEFORE reading anything ---")
    print("cwd =", os.getcwd())
    print("credentials =", os.path.abspath("credentials.json"))
    print("dict(session):", dict(session))
    ...
    flow.redirect_uri = url_for('user.google_callback', _external=True)
    
    if 'code_verifier' in session:
        flow.code_verifier = session['code_verifier']
        
    print("\n--- Before fetch_token() ---")
    print("flow.client_config:", flow.client_config)
    print("request.args:", request.args)
    print("request.url:", request.url)
    print("flow.redirect_uri:", getattr(flow, 'redirect_uri', None))
    print("flow.client_config:", getattr(flow, 'client_config', None))
    print("flow.code_verifier:", getattr(flow, 'code_verifier', None))
    print("session:", session)
    print("session['oauth_state']:", session.get('oauth_state'))
    ...
    existing_emails = EmailID.get_by_user(session['user_id'])
    for eid in existing_emails:
        if eid['email'] == email_address:
            return f"Email {email_address} is already connected.", 400
    ...
    result = EmailID.create(session['user_id'], email_data)
    return redirect(url_for('user.email_ids'))
...
def _send_emails_impl():
    ...
    for eid in user_email_ids:
        email_accounts.append({
            'email': eid['email'],
            'password': eid.get('password', ''),
            'smtp_server': eid.get('smtp_server'),
            'smtp_port': eid.get('smtp_port'),
            ...
            # OAuth fields
            'provider': eid.get('provider', 'smtp'),
            'refresh_token': eid.get('refresh_token'),
            'access_token': eid.get('access_token'),
            'token_uri': eid.get('token_uri'),
            'scopes': eid.get('scopes')
        })
    ...
    for r in recipients:
        ...
        personalized_recipients.append({'email': r['email'], 'body': personalized_body, 'status': r.get('status', 'VALID')})
    ...
    try:
        ...
        print("Selected Sender ID:", sender_email_id)
        
        selected_account = next((acc for acc in email_accounts if acc['_id'] == sender_email_id), None)
        if selected_account:
            print("Selected Account Email:", selected_account['email'])
            print("OAuth Credentials Loaded:", selected_account.get('provider') == 'google')
            print("Access Token Exists:", bool(selected_account.get('access_token')))
            print("Refresh Token Exists:", bool(selected_account.get('refresh_token')))
        else:
            print("Selected Account Not Found in email_accounts list!")
            
        print("Recipients Count:", len(recipients))
        print("CC List:", cc_emails)
